...
<head>
	<?php
	$supplier_name = $data_kanban->ed_supplier_name;
	if($supplier_name == "ADYAWINSA STAMPING INDUSTRIES"){
		$supplier_name = "ASI";
	}else{
		$supplier_name = "TMI";
	}
	...
	<title>(<?= $supplier_name; ?>) <?= $data_kanban->part_no; ?></title>
	<style type="text/css">
		* {
			margin: 0;
			padding: 0;
			...
			<table style="border-collapse:collapse;margin-left:5.991pt; margin-top: 5px; min-width: 98%;" cellspacing="0">
				<tr style="height:13pt">
					...
					<td style="width:304pt;border-top-style:solid;border-top-width:1pt;border-left-style:solid;border-left-width:1pt;border-bottom-style:solid;border-bottom-width:1pt;border-right-style:solid;border-right-width:1pt;text-align: center;vertical-align: middle;" colspan="2" rowspan="2">
						<svg id="barcode-'.$data_kanban->id.'-'.$i.'" jsbarcode-value="#'.$data_kanban->part_no.' 1 '.$data_kanban->po_ed.' '.$data_kanban->po_sto.' '.date('Ymd',strtotime($data_kanban->finish_delivery)).' '.sprintf('%04d',$i).'#" jsbarcode-width="1" jsbarcode-height="23" jsbarcode-format="code128" jsbarcode-displayValue="false" width="80%"></svg>
					</td>
				</tr>
